...
public class ResourceTypeRest {
	...
	public Response list(@QueryParam("filter") @DefaultValue("") String filter, 
		...
		try {
			...
			else
			{
				...
				JSONArray info = new ResourceTypeGenerator().generate(b.getServer());
	...
	public Response show(@PathParam("id") String id,
		...
		try {
			JSONBuilder b = new JSONBuilder(request);
			JSONObject info = new ResourceTypeGenerator().generate(b.getServer(), id);
			if (info == null)
